...
describe('ListUserEssaysStatisticsController (integration)', () => {
  ...
  describe('GET /api/essays/users/:username — error cases', () => {
    ...
    it("should return 403 when a STUDENT tries to view another user's essays", async () => {
      const student = await createUser({
        name: 'Aluno Teste',
        username: TEST_STUDENT_USERNAME,
        role: ROLES.STUDENT,
    ...
    it("should return 403 when a TEACHER tries to view another TEACHER's essays (equal role level)", async () => {
      const teacher = await createUser({
        name: 'Professor Teste',
        username: TEST_TEACHER_USERNAME,
        role: ROLES.TEACHER,
